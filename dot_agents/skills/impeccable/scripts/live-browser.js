...
(function () {
  ...
  const HELPER_ORIGIN =
    currentScript && currentScript.src
      ? new URL('.', new URL(currentScript.src, window.location.href)).href.replace(/\/$/, '')
      : 'http://localhost:' + PORT;
  ...
  function id8() {
    ...
    return Array.from(random, n => n.toString(16).padStart(8, '0')).join('').slice(0, 8);
  }
  ...
  async function applyEditing() {
    ...
    try {
      const res = await fetch(HELPER_ORIGIN + '/manual-edit-stash', {
        method: 'POST',
  ...
  async function fetchPendingCount() {
    try {
      const res = await fetch(
        HELPER_ORIGIN + '/manual-edit-stash?token=' + encodeURIComponent(TOKEN) + '&pageUrl=' + encodeURIComponent(location.pathname),
      );
      if (!res.ok) return;
  ...
  async function onPendingPillClick() {
    ...
    try {
      const res = await fetch(
        HELPER_ORIGIN + '/manual-edit-commit?token=' + encodeURIComponent(TOKEN) + '&pageUrl=' + encodeURIComponent(location.pathname) + '&async=1',
  ...
  async function onPendingTrashClick() {
    ...
    try {
      const res = await fetch(
        HELPER_ORIGIN + '/manual-edit-discard?token=' + encodeURIComponent(TOKEN) + '&pageUrl=' + encodeURIComponent(location.pathname),
        { method: 'POST' },
      );
  ...
  async function onPendingKeepFixingClick() {
    ...
    try {
      const res = await fetch(
        HELPER_ORIGIN + '/manual-edit-commit?token=' + encodeURIComponent(TOKEN) + '&pageUrl=' + encodeURIComponent(location.pathname) + '&async=1&repair=1',
        { method: 'POST', keepalive: true },
      );
      if (!res.ok) throw new Error('HTTP ' + res.status);
      if (pendingKeepFixingBtn) pendingKeepFixingBtn.style.display = 'none';
  ...
  async function onPendingRollbackClick() {
    ...
    try {
      const res = await fetch(
        HELPER_ORIGIN + '/manual-edit-repair-decision?token=' + encodeURIComponent(TOKEN) + '&pageUrl=' + encodeURIComponent(location.pathname),
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
  ...
  function injectVariantsFromSource(filePath, sessionId) {
    const url = HELPER_ORIGIN + '/source?token=' + TOKEN + '&path=' + encodeURIComponent(filePath);
    fetch(url)
  ...
  function sendEvent(msg, opts) {
    ...
    return fetch(HELPER_ORIGIN + '/events', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
...
void main() {
  ...
  function fetchAgentPollingStatus() {
    fetch(HELPER_ORIGIN + '/status?token=' + TOKEN, { cache: 'no-store' })
  ...
  async function fetchDesignSystem() {
    ...
    try {
      const [jsonRes, rawRes] = await Promise.all([
        fetch(`${HELPER_ORIGIN}/design-system.json?token=${TOKEN}`, { cache: 'no-store' }),
        fetch(`${HELPER_ORIGIN}/design-system/raw?token=${TOKEN}`, { cache: 'no-store' }),
      ]);
